...
import { sufferedRiskGaugeColors, sufferedRiskGaugeLabel } from "./presentational";

export function RiskSufferedGaugeSemicircle({ value }: { value: number }) {
  const v = Math.min(100, Math.max(0, value));
  const { arc, label: labelClass } = sufferedRiskGaugeColors(v);
